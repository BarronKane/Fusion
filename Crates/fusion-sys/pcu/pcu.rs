//! fusion-sys PCU composition and execution substrate.
//!
//! `fusion-sys::pcu` is the honest local/runtime layer above the PAL contract:
//! - it exposes the lowered Fusion-owned PCU IR and executor vocabulary
//! - it owns direct in-process planning/preparation/dispatch helpers
//! - it keeps honest SoC-specific lanes such as Cortex-M PIO available
//! - it does not require channels for local composition, even though the same protocol surfaces
//!   can later ride over channels or other transports when boundaries are crossed
...
/// `fusion-sys` PCU facade plus one reusable dispatch profile.
...
/// Returns the public `fusion-sys` PCU facade for the selected backend.
#[must_use]
