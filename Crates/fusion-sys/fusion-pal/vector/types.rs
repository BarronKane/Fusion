...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum SlotState {
    /// Fusion does not own this slot in the current mode.
    Foreign,
    /// The slot is currently unbound under Fusion ownership.
    Unbound,
    /// The slot is bound inline.
    Inline {
        priority: Option<VectorPriority>,
        core: Option<ThreadCoreId>,
        stack: VectorInlineStackKind,
    },
    /// The slot is bound to one deferred dispatch lane.
    Deferred {
        lane: VectorDispatchLane,
        cookie: VectorDispatchCookie,
        priority: Option<VectorPriority>,
