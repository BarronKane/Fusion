...
impl VectorBase for CortexMVector {
    fn support(&self) -> VectorSupport {
        let slot_count = u16::try_from(super::super::hal::soc::board::irqs().len()).unwrap_or(0);
...
impl VectorTableBuilderControl for CortexMVectorBuilder {
    ...
    fn bind(&mut self, binding: VectorSlotBinding) -> Result<(), VectorError> {
        ...
        if let Some(priority) = binding.priority {
            super::super::hal::soc::board::irq_set_priority(binding.slot.0, priority.0)
...
pub fn take_pending_active_scope(
    ...
    let slot_count = u16::try_from(super::super::hal::soc::board::irqs().len())
...
const fn map_hardware_error(error: crate::pal::hal::HardwareError) -> VectorError {
    match error.kind() {
        crate::pal::hal::HardwareErrorKind::Unsupported => VectorError::unsupported(),
        crate::pal::hal::HardwareErrorKind::Invalid => VectorError::invalid(),
        crate::pal::hal::HardwareErrorKind::ResourceExhausted => VectorError::resource_exhausted(),
        crate::pal::hal::HardwareErrorKind::StateConflict => VectorError::state_conflict(),
        crate::pal::hal::HardwareErrorKind::Busy => VectorError::state_conflict(),
        crate::pal::hal::HardwareErrorKind::Platform(code) => VectorError::platform(code),
    }
}

fn per_core_tables_supported() -> bool {
    super::super::hal::soc::board::selected_soc()
        .topology_summary
...
fn slot_descriptor(
    slot: IrqSlot,
) -> Result<&'static super::super::hal::soc::board::CortexMIrqDescriptor, VectorError> {
...
fn take_pending_cookies(
    ...
    for slot in 0..usize::from(slot_count) {
        ...
        match meta.state {
            ...
            SlotState::Foreign | SlotState::Unbound | SlotState::Deferred { .. } => {}
        }
    }
    Ok(written)
}
...
mod tests {
    ...
    fn test_builder() -> CortexMVectorBuilder {
        CortexMVectorBuilder {
            ...
            slot_count: u16::try_from(super::super::hal::soc::board::irqs().len())
