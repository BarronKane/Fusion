//! Native `aarch64` Windows context-switch backend.
//!
//! This backend saves the Windows Arm64 callee-saved integer register set, the preserved SIMD
//! lanes, and floating-point control state directly in an ISA-specific record.
...
/// Native `aarch64` Windows context provider.
...
unsafe extern "C" {
    fn fusion_windows_aarch64_context_swap(
...
impl WindowsContext {
    /// Creates a new `aarch64` Windows context provider.
    #[must_use]
...
// SAFETY: this backend saves and restores the reported Windows Arm64 context record directly and
// the saved state is carrier-agnostic process memory, so suspended contexts may resume on another
...
unsafe impl ContextSwitch for WindowsContext {
    unsafe fn make(
        ...
        saved.registers.x30 = fusion_windows_aarch64_context_start as *const () as usize;
        saved.ready = true;
    ...
    unsafe fn swap(
        ...
        from.ready = true;
        unsafe {
            fusion_windows_aarch64_context_swap(&raw mut from.registers, &raw const to.registers);
        }
        Ok(())
    }
}
...
#[unsafe(no_mangle)]
extern "C" fn fusion_windows_aarch64_context_bootstrap(entry_bits: usize, arg_bits: usize) -> ! {
...
unsafe extern "C" {
    fn fusion_windows_aarch64_context_start() -> !;
}
