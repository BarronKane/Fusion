...
impl VulkanRHI {
    pub(crate) fn vk_find_queue_families<'a>(
        instance: &ash::Instance,
        device: &PhysicalDevice,
    ) -> Result<Vec<QueueFamilyProperties2<'a>>> {
    ...
    pub(crate) fn vk_is_device_suitable(
        instance: &ash::Instance,
        device: &PhysicalDevice,
    ) -> Result<bool> {
        ...
        let has_graphics_family = queue_families.iter().any(|q| {
        ...
        Ok(
            device_properties.properties.device_type == vk::PhysicalDeviceType::DISCRETE_GPU
    ...
    pub(crate) fn vk_create_logical_device(
        instance: &ash::Instance,
        ...
        let device_features2 = PhysicalDeviceFeatures2::default().features(device_features);
