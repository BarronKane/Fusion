#![allow(
    ...
    unused_unsafe,
    unused_variables,
    reason = "Vulkan backend is an active low-level spike with intentionally staged implementation."
)]
#![allow(
    clippy::all,
    clippy::nursery,
    clippy::pedantic,
    reason = "Vulkan backend warnings are triaged separately while API and memory model are still being designed."
)]
...
use fusion_rhi_core::{AppInfo, RHI};
...
impl VulkanRHIBuilder {
    ...
    pub(crate) fn vk_create_instance(
        app_info: &AppInfo,
        entry: &ash::Entry,
        ...
        let mut vk_create_info: vk::InstanceCreateInfo =
        ...
        if with_validation {
            ...
            match validation_layers_result {
                Ok(v) => {
                    validation_layers = v;
                    vk_create_info =
                        vk_create_info.enabled_layer_names(&validation_layers.as_slice());
                }
                Err(e) => {
        ...
        let instance = unsafe { entry.create_instance(&vk_create_info, None) };
    ...
    pub(crate) fn vk_check_validation_layer_support(
        entry: &ash::Entry,
        layers: Vec<*const core::ffi::c_char>,
    ) -> Result<bool> {
    ...
    fn set_debug_type(
        ...
        if general {
            debug_type |= vk::DebugUtilsMessageTypeFlagsEXT::GENERAL;
        }
        if validation {
            debug_type |= vk::DebugUtilsMessageTypeFlagsEXT::VALIDATION;
        }
        if performance {
            debug_type |= vk::DebugUtilsMessageTypeFlagsEXT::PERFORMANCE;
        }
...
impl RHI<VulkanRHIBuilder, VulkanRHI> for VulkanRHI {
    fn init(builder: &VulkanRHIBuilder) -> Result<Self> {
        ...
        let with_validation_result = VulkanRHIBuilder::vk_check_validation_layer_support(
            &entry,
            VulkanRHIBuilder::create_validation_layers()?,
        );
        ...
        let instance =
            VulkanRHIBuilder::vk_create_instance(&builder.app_info, &entry, with_validation)?;
