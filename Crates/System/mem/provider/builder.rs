...
pub enum MemoryProviderDiscoveryPolicy {
    ...
    ExplicitOnly,
    /// Merge PAL discovery with caller-supplied overlays.
...
pub enum MemoryProviderConflictPolicy {
    ...
    PreferExplicit,
    /// Prefer descriptors discovered through the PAL.
    PreferDiscovered,
}
...
/// default shape is "discover from PAL, then merge explicit overlays if the caller supplied
/// them." That lets a hosted target use only normalized PAL discovery, while a DMA-heavy
/// board or firmware environment can inject additional bound resources or topology that the
/// PAL cannot discover automatically.
...
impl MemoryProviderBuildSpec<'_> {
    ...
    /// Returns an explicit-only composition policy with no PAL discovery.
    #[must_use]
