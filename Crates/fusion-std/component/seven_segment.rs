//! Four-digit multiplexed seven-segment display components backed by owned GPIO outputs.
//!
//! The display owns one GPIO per segment line (`a` through `g`, plus decimal point) and one
//! GPIO per digit-select line (`d1` through `d4`). The component maintains one four-glyph
