//! 3.5mm audio jack module backed by owned GPIO lines.
//!
...
//! this component owns the pins and exposes the wiring truth for each conductor.
...
/// TRRS 3.5mm audio jack module backed by four owned GPIO outputs.
///
/// Each conductor (tip, ring1, ring2, sleeve) is independently owned. The actual audio signal
/// generation is left to a future DAC, PWM, or PIO/I2S contract; this component establishes
...
{
    /// Creates one TRRS audio jack component by claiming all four conductor pins.
    ///
...
/// TRS 3.5mm audio jack module backed by three owned GPIO outputs.
///
...
where
    ...
    Sleeve: GpioOutputPin,
{
    /// Creates one TRS audio jack component by claiming the tip, ring, and sleeve pins.
    ///
    /// # Errors
    ///
    /// Returns one honest GPIO error when any conductor pin cannot be configured for output.
...
mod tests {
    use super::*;
    use fusion_sys::gpio::{GpioCapabilities, GpioOwnedPin};
