//! Passive speaker/woofer component backed by owned GPIO output.
//!
...
//! this component owns the signal pin and exposes the wiring truth.

use fusion_sys::gpio::{GpioError, GpioOutputPin};

/// Passive enclosed speaker backed by one owned GPIO signal output.
///
...
/// frequency output. This component owns the pin and tracks the commanded state.
#[derive(Debug)]
...
where
    P: GpioOutputPin,
{
    /// Creates one passive speaker component by claiming the signal pin.
    ///
