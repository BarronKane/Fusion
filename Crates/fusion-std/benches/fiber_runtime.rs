...
fn thread_async_runtime(worker_count: usize) -> ThreadAsyncRuntime {
...
#[bench]
#[ignore = "cross-thread wake latency is benchmarked in isolation because the full bench sweep makes this probe flaky"]
fn current_async_runtime_cross_thread_wake_once(b: &mut Bencher) {
    let runtime = CurrentAsyncRuntime::new();
...
#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_noop_workers_1(b: &mut Bencher) {
    bench_thread_async_runtime_lifecycle_throughput(b, 1, async_noop_job);
}

#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_noop_workers_2(b: &mut Bencher) {
    bench_thread_async_runtime_lifecycle_throughput(b, 2, async_noop_job);
}

#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_noop_workers_4(b: &mut Bencher) {
    bench_thread_async_runtime_lifecycle_throughput(b, 4, async_noop_job);
}

#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_yield_once_workers_1(b: &mut Bencher) {
    bench_thread_async_runtime_lifecycle_throughput(b, 1, async_yield_once_job);
}

#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_yield_once_workers_2(b: &mut Bencher) {
    bench_thread_async_runtime_lifecycle_throughput(b, 2, async_yield_once_job);
}

#[bench]
#[ignore = "hosted thread-backed async currently segfaults under repeated lifecycle bench pressure"]
fn thread_async_runtime_lifecycle_yield_once_workers_4(b: &mut Bencher) {
...
#[bench]
#[ignore = "cross-thread wake latency is benchmarked in isolation because the full bench sweep makes this probe flaky"]
fn tokio_current_thread_cross_thread_wake_once(b: &mut Bencher) {
    let runtime = TokioRuntimeBuilder::new_current_thread()
        .build()
        .expect("tokio current-thread runtime should build for benches");
    let (tx, worker) = cross_thread_wake_worker();
...
#[bench]
fn tokio_multi_thread_lifecycle_noop_workers_1(b: &mut Bencher) {
    bench_tokio_multi_thread_lifecycle_throughput(b, 1, tokio_async_noop_job);
}

#[bench]
fn tokio_multi_thread_lifecycle_noop_workers_2(b: &mut Bencher) {
    bench_tokio_multi_thread_lifecycle_throughput(b, 2, tokio_async_noop_job);
}

#[bench]
fn tokio_multi_thread_lifecycle_noop_workers_4(b: &mut Bencher) {
    bench_tokio_multi_thread_lifecycle_throughput(b, 4, tokio_async_noop_job);
}

#[bench]
fn tokio_multi_thread_lifecycle_yield_once_workers_1(b: &mut Bencher) {
    bench_tokio_multi_thread_lifecycle_throughput(b, 1, tokio_async_yield_once_job);
}

#[bench]
fn tokio_multi_thread_lifecycle_yield_once_workers_2(b: &mut Bencher) {
    bench_tokio_multi_thread_lifecycle_throughput(b, 2, tokio_async_yield_once_job);
}

#[bench]
fn tokio_multi_thread_lifecycle_yield_once_workers_4(b: &mut Bencher) {
...
#[bench]
fn current_async_runtime_contention_yield_32x32(b: &mut Bencher) {
    let runtime = CurrentAsyncRuntime::new();
...
#[bench]
fn tokio_current_thread_contention_yield_32x32(b: &mut Bencher) {
    let runtime = TokioRuntimeBuilder::new_current_thread()
        .build()
        .expect("tokio current-thread runtime should build for benches");
...
#[bench]
fn reactor_readiness_batch_16(b: &mut Bencher) {
    bench_reactor_batch_ready(b, REACTOR_BATCH_SMALL);
}

#[bench]
fn reactor_readiness_batch_64(b: &mut Bencher) {
...
fn cross_thread_wake_worker() -> (SyncSender<Arc<CrossThreadWakeState>>, JoinHandle<()>) {
...
fn cross_thread_wake_round_fusion(
...
fn cross_thread_wake_round_tokio_current(
...
fn current_async_contention_round(runtime: &CurrentAsyncRuntime) {
...
fn tokio_current_contention_round(runtime: &tokio::runtime::Runtime) {
    runtime.block_on(async {
        let mut handles = Vec::with_capacity(ASYNC_CONTENTION_TASKS);
        for _ in 0..ASYNC_CONTENTION_TASKS {
...
fn bench_thread_async_runtime_lifecycle_throughput<F, Fut>(
    b: &mut Bencher,
    worker_count: usize,
    future_factory: F,
) where
...
fn bench_tokio_multi_thread_lifecycle_throughput<F, Fut>(
    b: &mut Bencher,
    worker_count: usize,
    future_factory: F,
) where
...
fn bench_reactor_batch_ready(b: &mut Bencher, source_count: usize) {
